    ...
    return (
      <View className="page-view-outer">
            ...
            cardList.map((item, index) => item.className !== 'none' ? (
              ...
              >
                <View className="card-image-view">
                  <Image mode="aspectFill" className="card-image" src={item.image} onClick={() => this.previewImg(item)}></Image>
                </View>
                <View className="card-fixed-bottom">
                  {item.title}
                </View>
              </MovableView>
            ) : null)
        ...
        <View className="background-tips">
          <View className="tips-item">～～～滑动卡片实例～～～</View>
          <View className="tips-item">～～～来自知乎日报api～～～</View>
        </View>
      </View>
    )
  }
